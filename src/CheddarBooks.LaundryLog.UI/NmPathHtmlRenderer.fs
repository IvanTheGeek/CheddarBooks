...
module NmPathHtmlRenderer =
    ...
    let private renderScript (builder: StringBuilder) (pathState: NmPathState) (updateState: NmPathUpdateState option) =
        ...
        appendLine builder "    return new Intl.DateTimeFormat(undefined, { hour: 'numeric', minute: '2-digit', timeZoneName: 'short' }).format(parsed);"
        appendLine builder "  };"
        ...
        appendLine builder "      updateStatus.textContent = `${lead}UPDATED AT: ${formattedClock} | ${formattedAge}`;"
        appendLine builder "      setUpdateStatusState(state);"
        appendLine builder "    };"
