...
module ScreenPathHtmlRenderer =
    ...
    let private renderPathStyles (builder: StringBuilder) =
        ...
        appendLine builder ".ll-path-nav-button__icon { font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', Menlo, monospace; font-size: 1.15rem; letter-spacing: -0.04em; line-height: 1; }"
    ...
    let private renderPathScript (builder: StringBuilder) =
        ...
        appendLine builder "  const stepOffsets = () => Array.from(flow.querySelectorAll('.ll-path-step')).map((step) => step.offsetLeft).sort((left, right) => left - right);"
        appendLine builder "  const clampLeft = (left) => Math.max(0, Math.min(left, maxScrollLeft()));"
        appendLine builder "  const stepIndexAtOrBefore = (offsets, currentLeft) => {"
        appendLine builder "    let index = 0;"
        appendLine builder "    for (let i = 0; i < offsets.length; i += 1) {"
        appendLine builder "      if (offsets[i] <= currentLeft + 4) { index = i; }"
        appendLine builder "    }"
        appendLine builder "    return index;"
        appendLine builder "  };"
        appendLine builder "  const scrollToColumn = (targetLeft) => viewport.scrollTo({ left: clampLeft(targetLeft), behavior: 'smooth' });"
        ...
        appendLine builder "    const contentWidth = Math.max(flow.scrollWidth, Math.ceil(flow.getBoundingClientRect().width));"
        appendLine builder "    scrollbarContent.style.width = `${contentWidth}px`;"
        ...
        appendLine builder "    updateButtonState();"
        appendLine builder "  });"
        appendLine builder "  const scrollToStart = () => scrollToColumn(0);"
        appendLine builder "  const scrollToEnd = () => scrollToColumn(maxScrollLeft());"
        appendLine builder "  const scrollByOneColumn = (direction) => {"
        appendLine builder "    const offsets = stepOffsets();"
        appendLine builder "    if (offsets.length === 0) { return; }"
        appendLine builder "    const currentIndex = stepIndexAtOrBefore(offsets, viewport.scrollLeft);"
        appendLine builder "    const targetIndex = Math.max(0, Math.min(offsets.length - 1, currentIndex + direction));"
        appendLine builder "    scrollToColumn(offsets[targetIndex]);"
        appendLine builder "  };"
    ...
    let renderDocument (screenPathState: ScreenPathState) =
        ...
        appendLine builder "<button id=\"ll-path-nav-start\" class=\"ll-path-nav-button\" type=\"button\" aria-label=\"Back to beginning\"><span class=\"ll-path-nav-button__icon\">||&lt;</span></button>"
        appendLine builder "<button id=\"ll-path-nav-previous\" class=\"ll-path-nav-button\" type=\"button\" aria-label=\"Show the previous screen column\"><span class=\"ll-path-nav-button__icon\">|&lt;</span></button>"
        ...
        appendLine builder "</div>"
        appendLine builder "<button id=\"ll-path-nav-next\" class=\"ll-path-nav-button\" type=\"button\" aria-label=\"Show the next screen column\"><span class=\"ll-path-nav-button__icon\">&gt;|</span></button>"
        appendLine builder "<button id=\"ll-path-nav-end\" class=\"ll-path-nav-button\" type=\"button\" aria-label=\"Go to end\"><span class=\"ll-path-nav-button__icon\">&gt;||</span></button>"
        appendLine builder "</section>"
