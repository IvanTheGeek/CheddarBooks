...
module SliceHtmlRenderer =
    ...
    let private renderStyles (builder: StringBuilder) =
        ...
        appendLine builder ".path-document { padding: 16px 18px 18px; }"
        appendLine builder ".path-document__header { max-width: 980px; margin-bottom: 12px; }"
        ...
        appendLine builder ".path-document__title { margin: 0; font-size: 1.65rem; line-height: 1.08; }"
        appendLine builder ".path-document__description { margin: 7px 0 0; max-width: 920px; font-size: 0.92rem; line-height: 1.45; color: #48627f; }"
        appendLine builder ".path-document__row { display: flex; gap: 16px; overflow-x: auto; align-items: stretch; padding: 4px 2px 8px; }"
        appendLine builder ".slice-card { flex: 0 0 276px; min-height: 362px; border-radius: 20px; border: 4px solid #15263d; box-shadow: 0 10px 24px rgba(10, 27, 49, 0.1); padding: 10px 10px 11px; display: flex; flex-direction: column; }"
        ...
        appendLine builder ".slice-card__app-pill { display: inline-flex; align-items: center; justify-content: center; padding: 4px 10px; border-radius: 999px; background: #ffb54d; color: white; font-size: 0.72rem; font-weight: 700; line-height: 1; }"
        appendLine builder ".slice-card__kind { color: #0e5883; font-size: 0.72rem; font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; text-align: right; }"
        appendLine builder ".slice-card__title { margin: 10px 0 0; font-size: 1.04rem; line-height: 1.05; }"
        appendLine builder ".slice-card__description { margin: 6px 0 0; font-size: 0.8rem; color: #506b87; min-height: 2.05em; }"
        appendLine builder ".slice-card__body { display: grid; grid-template-rows: 84px 84px 84px; gap: 9px; margin-top: 12px; flex: 1; align-content: start; }"
        appendLine builder ".slice-card__slot--empty { border-radius: 16px; background: transparent; }"
        appendLine builder ".slice-block { display: flex; flex-direction: column; gap: 5px; border-radius: 15px; padding: 7px 8px; border: 2px solid; min-height: 0; overflow: hidden; }"
        ...
        appendLine builder ".slice-block__topline { position: relative; min-height: 20px; padding-right: 74px; }"
        appendLine builder ".slice-block__title { margin: 10px 0 0; font-size: 0.73rem; line-height: 1.15; font-weight: 700; overflow-wrap: anywhere; }"
        appendLine builder ".slice-block--screen .slice-block__title { font-weight: 400; margin-top: 15px; }"
        appendLine builder ".slice-block__badges { position: absolute; top: 0; right: 0; display: flex; flex-direction: column; align-items: flex-end; gap: 4px; }"
        appendLine builder ".slice-block__badge { display: inline-flex; align-items: center; justify-content: center; padding: 2px 7px; border-radius: 999px; border: 1px solid #c2d4e8; background: rgba(255, 255, 255, 0.92); font-size: 0.55rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #4a647f; white-space: nowrap; }"
        appendLine builder ".slice-block__badge--kind { color: #203954; }"
        appendLine builder ".slice-block__snapshot { min-height: 32px; border-radius: 12px; border: 1px dashed #c7d7ea; background: linear-gradient(180deg, #eef4fb 0%, #e7eef7 100%); display: flex; align-items: center; justify-content: center; text-align: center; padding: 8px; color: #7087a0; font-size: 0.75rem; font-weight: 600; }"
        appendLine builder ".slice-block__properties { display: flex; flex-direction: column; gap: 2px; background: rgba(255, 255, 255, 0.32); border-radius: 10px; padding: 6px 7px 5px; min-height: 0; max-height: 44px; overflow: hidden; }"
        appendLine builder ".slice-block__properties--hidden { min-height: 0; padding: 0; background: transparent; }"
        appendLine builder ".slice-block__property-line { font-family: \"IBM Plex Mono\", \"Cascadia Mono\", \"Consolas\", monospace; font-size: 0.62rem; line-height: 1.16; color: #27435c; overflow-wrap: anywhere; word-break: break-word; }"
        appendLine builder ".slice-block__footer { display: flex; justify-content: flex-end; margin-top: auto; }"
        appendLine builder ".slice-block__footer-badge { display: inline-flex; align-items: center; justify-content: center; padding: 2px 7px; border-radius: 999px; border: 1px solid #c6d4e2; background: rgba(255, 255, 255, 0.94); color: #566d86; font-size: 0.55rem; font-weight: 700; letter-spacing: 0.08em; text-transform: lowercase; white-space: nowrap; }"
        appendLine builder "@media (max-width: 1200px) { .slice-card { flex-basis: 258px; min-height: 346px; } .path-document { padding-left: 14px; padding-right: 14px; } }"
        appendLine builder "@media (max-width: 900px) { .path-document__row { gap: 12px; } .slice-card { flex-basis: 244px; min-height: 338px; } }"
        appendLine builder "</style>"
