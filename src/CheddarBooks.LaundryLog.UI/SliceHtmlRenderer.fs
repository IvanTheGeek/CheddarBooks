...
module SliceHtmlRenderer =
    ...
    let private renderScript (builder: StringBuilder) =
        ...
        appendLine builder "  const pathDocument = document.querySelector('.path-document');"
        ...
        appendLine builder "  const widthButton = document.querySelector('[data-action=\"toggle-property-width\"]');"
        ...
        appendLine builder "    });"
        appendLine builder "  }"
        appendLine builder "  if (widthButton && pathDocument) {"
        appendLine builder "    const updateWidthButton = function () {"
        appendLine builder "      const expanded = pathDocument.classList.contains('path-document--properties-wide');"
        appendLine builder "      widthButton.textContent = expanded ? 'Collapse Property Width' : 'Expand Property Width';"
        appendLine builder "      widthButton.setAttribute('aria-pressed', expanded ? 'true' : 'false');"
        appendLine builder "    };"
        appendLine builder "    updateWidthButton();"
        appendLine builder "    widthButton.addEventListener('click', function () {"
        appendLine builder "      pathDocument.classList.toggle('path-document--properties-wide');"
        appendLine builder "      updateWidthButton();"
        appendLine builder "    });"
        appendLine builder "  }"
        appendLine builder "});"
    ...
    let private renderStyles (builder: StringBuilder) =
        ...
        appendLine builder ".path-document--properties-wide .slice-card { grid-column: span 2; }"
    ...
    let renderDocument options (pathRow: PathRowState) =
        ...
        appendLine builder $"<main class=\"path-document\" style=\"--slice-columns: {pathRow.SliceCards.Length}; --slice-wide-columns: {pathRow.SliceCards.Length * 2};\">"
        ...
        appendLine builder "<button type=\"button\" class=\"path-document__action\" data-action=\"toggle-property-width\" aria-pressed=\"false\">Expand Property Width</button>"
        appendLine builder "</div>"
