...
module SliceHtmlRenderer =
    ...
    let private renderBlockContent (builder: StringBuilder) showProperties content =
        ...
        | PropertyLines propertyLines when showProperties ->
            appendLine builder "<details class=\"slice-block__properties-panel\" open>"
    ...
    let private renderStyles (builder: StringBuilder) =
        ...
        appendLine builder ".path-document { padding: 10px 12px 12px; }"
        appendLine builder ".path-document__header { max-width: 900px; margin-bottom: 8px; }"
        appendLine builder ".path-document__eyebrow { font-size: 0.68rem; letter-spacing: 0.16em; font-weight: 700; color: #56718d; text-transform: uppercase; margin-bottom: 6px; }"
        appendLine builder ".path-document__title { margin: 0; font-size: 1.38rem; line-height: 1.05; }"
        appendLine builder ".path-document__description { margin: 5px 0 0; max-width: 860px; font-size: 0.8rem; line-height: 1.35; color: #48627f; }"
        appendLine builder ".path-document__row { display: flex; gap: 12px; overflow-x: auto; align-items: stretch; padding: 2px 2px 6px; }"
        appendLine builder ".slice-card { flex: 0 0 248px; min-height: 336px; border-radius: 20px; border: 4px solid #15263d; box-shadow: 0 10px 24px rgba(10, 27, 49, 0.1); padding: 8px 8px 9px; display: flex; flex-direction: column; }"
        ...
        appendLine builder ".slice-card__app-pill { display: inline-flex; align-items: center; justify-content: center; padding: 3px 9px; border-radius: 999px; background: #ffb54d; color: white; font-size: 0.64rem; font-weight: 700; line-height: 1; }"
        appendLine builder ".slice-card__kind { color: #0e5883; font-size: 0.62rem; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; text-align: right; }"
        appendLine builder ".slice-card__title { margin: 8px 0 0; font-size: 0.96rem; line-height: 1.03; }"
        appendLine builder ".slice-card__description { margin: 4px 0 0; font-size: 0.72rem; color: #506b87; min-height: 1.8em; }"
        appendLine builder ".slice-card__body { display: grid; grid-template-rows: 82px 82px 82px; gap: 8px; margin-top: 9px; flex: 1; align-content: start; }"
        appendLine builder ".slice-card__slot--empty { border-radius: 16px; background: transparent; }"
        appendLine builder ".slice-block { display: flex; flex-direction: column; gap: 4px; border-radius: 14px; padding: 6px 7px; border: 2px solid; min-height: 0; overflow: hidden; }"
        ...
        appendLine builder ".slice-block__topline { position: relative; min-height: 18px; padding-right: 106px; }"
        appendLine builder ".slice-block__title { margin: 8px 0 0; font-size: 0.66rem; line-height: 1.13; font-weight: 700; overflow-wrap: anywhere; }"
        appendLine builder ".slice-block--screen .slice-block__title { font-weight: 400; margin-top: 14px; }"
        appendLine builder ".slice-block__badges { position: absolute; top: 0; right: 0; display: flex; flex-direction: row; justify-content: flex-end; gap: 4px; width: 102px; }"
        appendLine builder ".slice-block__badge { display: inline-flex; align-items: center; justify-content: center; padding: 2px 7px; border-radius: 999px; border: 1px solid #c2d4e8; background: rgba(255, 255, 255, 0.92); font-size: 0.5rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #4a647f; white-space: nowrap; }"
        appendLine builder ".slice-block__badge--kind { color: #203954; }"
        appendLine builder ".slice-block__snapshot { min-height: 24px; border-radius: 10px; border: 1px dashed #c7d7ea; background: linear-gradient(180deg, #eef4fb 0%, #e7eef7 100%); display: flex; align-items: center; justify-content: center; text-align: center; padding: 6px; color: #7087a0; font-size: 0.66rem; font-weight: 600; }"
        appendLine builder ".slice-block__properties-panel { margin: 0; display: flex; flex-direction: column; gap: 3px; flex: 1; min-height: 0; }"
        ...
        appendLine builder ".slice-block__properties-summary { align-self: flex-end; display: inline-flex; align-items: center; justify-content: center; padding: 1px 6px; border-radius: 999px; border: 1px solid #c6d4e2; background: rgba(255, 255, 255, 0.94); color: #566d86; font-size: 0.48rem; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; }"
        appendLine builder ".slice-block__properties { display: flex; flex-direction: column; gap: 2px; background: rgba(255, 255, 255, 0.32); border-radius: 9px; padding: 6px 7px 5px; min-height: 0; flex: 1; overflow: hidden; }"
        appendLine builder ".slice-block__properties--hidden { min-height: 0; padding: 0; background: transparent; }"
        appendLine builder ".slice-block__property-line { font-family: \"IBM Plex Mono\", \"Cascadia Mono\", \"Consolas\", monospace; font-size: 0.56rem; line-height: 1.12; color: #27435c; overflow-wrap: anywhere; word-break: break-word; }"
        appendLine builder ".slice-block__footer { display: flex; justify-content: flex-end; margin-top: auto; }"
        appendLine builder ".slice-block__footer-badge { display: inline-flex; align-items: center; justify-content: center; padding: 2px 7px; border-radius: 999px; border: 1px solid #c6d4e2; background: rgba(255, 255, 255, 0.94); color: #566d86; font-size: 0.5rem; font-weight: 700; letter-spacing: 0.08em; text-transform: lowercase; white-space: nowrap; }"
        appendLine builder "@media (max-width: 1200px) { .slice-card { flex-basis: 236px; min-height: 328px; } .path-document { padding-left: 10px; padding-right: 10px; } }"
        appendLine builder "@media (max-width: 900px) { .path-document__row { gap: 10px; } .slice-card { flex-basis: 224px; min-height: 322px; } }"
        appendLine builder "</style>"
