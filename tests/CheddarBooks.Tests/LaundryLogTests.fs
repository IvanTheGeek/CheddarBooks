        ...
        testList
            ...
            [ testCase "Expense kinds map to stable slugs" (fun () ->
              ...
              testCase "NM path renderer sequences the first ATLAS-style mixed path" (fun () ->
                  ...
                  Expect.stringContains htmlDocument "data-slot-kind=\"screen\"" "Expected NM columns to emit a shared screen slot."
                  ...
                  Expect.stringContains htmlDocument "data-testid=\"slice-kind-trigger\"" "Expected embedded AEM block classifications to expose stable help triggers."
                  Expect.stringContains htmlDocument "data-testid=\"slice-gwt-kind-trigger\"" "Expected embedded AEM GWT classifications to expose stable help triggers."
                  Expect.stringContains htmlDocument "Column Classification" "Expected whole-column classification popovers to explain the NM column kind explicitly."
                  Expect.stringContains htmlDocument "Compartment Classification" "Expected compartment popovers to explain the local classification explicitly."
                  Expect.stringContains htmlDocument "Bounded Context" "Expected the popovers to identify bounded contexts explicitly."
                  Expect.stringContains htmlDocument "Lens" "Expected the popovers to identify lenses explicitly."
                  Expect.stringContains htmlDocument "Actor" "Expected the popovers to identify actors explicitly."
                  Expect.isFalse (htmlDocument.Contains("Bounded Context badge")) "Expected the popover headers to drop the extra word 'badge' for bounded contexts."
                  Expect.isFalse (htmlDocument.Contains("Lens badge")) "Expected the popover headers to drop the extra word 'badge' for lenses."
                  Expect.isFalse (htmlDocument.Contains("Actor badge")) "Expected the popover headers to drop the extra word 'badge' for actors."
